...
from utils import accuracy, build_optimizer
...
class Explainer:
    ...
    def explain_graph(
        ...
        x = torch.tensor(self.feat, requires_grad=True, dtype=torch.double)
        ...
        if model == "exp":
            ...
            for epoch in range(self.num_epochs):
                ...
                mask_density = explainer.mask_density()
                feat_mask_density = explainer.feat_mask_density()
...
class ExplainModule(nn.Module):
    def __init__(
        ...
        self.scheduler, self.optimizer = build_optimizer(params)
    ...
    def construct_feat_mask(self, feat_dim, init_strategy="normal", const_val=0):
        mask = nn.Parameter(torch.FloatTensor(feat_dim), requires_grad=True)
    ...
    def construct_edge_mask(self, num_nodes, init_strategy="const", const_val=0):
        mask = nn.Parameter(torch.FloatTensor(num_nodes, num_nodes), requires_grad=True)
        if init_strategy == "normal":
    ...
    def feat_mask_density(self):
        ...
        mask_sum = torch.mean(feat_mask).cpu()
        # print(feat_mask)
        return mask_sum

    def forward(self, node_idx=0, mask_features=True, marginalize=False):
        x = self.x
    ...
    def loss(self, pred, pred_label, gt_label=None, node_idx=0):
        """
        Args:
            pred: the label prediction made by original model
            pred_label: predicted by the current model.
        """
        if self.graph_mode:
            # pred_label_node = pred_label
            # gt_label_node = self.label
            # logit = pred_label[gt_label_node]
